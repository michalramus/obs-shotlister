...
describe('formatMs', () => {
  ...
  it('formats 1000ms as 0:01.0', () => {
    expect(formatMs(1000)).toBe('0:01.0')
  })

  it('formats 60000ms as 1:00.0', () => {
    expect(formatMs(60000)).toBe('1:00.0')
  })

  it('formats 90500ms as 1:30.5', () => {
    expect(formatMs(90500)).toBe('1:30.5')
  })

  it('formats 3661000ms as 61:01.0', () => {
    expect(formatMs(3661000)).toBe('61:01.0')
  })

  it('clamps negative values to 0:00.0', () => {
    expect(formatMs(-5000)).toBe('0:00.0')
  })
})
...
describe('computeTiming — no filter', () => {
  ...
  it('includes intermediate shot durations in timeUntilLive', () => {
    ...
    const cameras = [makeCamera('cam-1', 1), makeCamera('cam-2', 2), makeCamera('cam-3', 3)]
...
describe('computeTiming — effectiveDurationMs', () => {
  it('equals live shot duration when no hidden shots follow', () => {
    const shots = [makeShot('s1', 'cam-1', 10000, 0), makeShot('s2', 'cam-2', 5000, 1)]
    const cameras = [makeCamera('cam-1', 1), makeCamera('cam-2', 2)]
    const result = computeTiming(shots, cameras, 0, 0, 2000)
    expect(result.effectiveDurationMs).toBe(10000)
  })
